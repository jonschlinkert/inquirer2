...
ScreenManager.prototype.render = function (content, opt) {
  ...
  var rawPromptLine = utils.stripAnsi(promptLine);
  ...
  var rawPrompt = utils.stripAnsi(prompt);
  ...
  var actualLines = utils._.flatten(breakedLines);
  ...
  util.left(this.rl, utils.stripAnsi(utils._.last(actualLines)).length);
  ...
  this.extraLinesUnderPrompt = utils._.flatten(bottomSection).length;
