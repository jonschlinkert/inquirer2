...
function Prompt() {
  ...
  if (utils._.isArray(this.opt.default)) {
...
Prompt.prototype.getCurrentValue = function () {
  ...
  this.selection = utils._.pluck(choices, 'short');
  return utils._.pluck(choices, 'value');
};
...
function renderChoices(choices, pointer) {
  ...
  choices.forEach(function (choice, i) {
    ...
    if (choice.disabled) {
      separatorOffset++;
      output += ' - ' + choice.name;
      output += ' (' + (utils._.isString(choice.disabled) ? choice.disabled : 'Disabled') + ')';
    } else {
